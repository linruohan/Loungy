...
use crate::{components::list::Item, state::LazyMutex};
...
pub fn fuzzy_match<T: Score>(pattern: &str, items: Vec<T>, path: bool) -> Vec<T> {
    ...
    let pattern =
        Atom::new(pattern, CaseMatching::Smart, Normalization::Smart, AtomKind::Fuzzy, false);
    let mut buf = Vec::new();
    let mut items: Vec<(T, u16)> =
        items.into_iter().filter_map(|item| item.score(&pattern, &mut buf, &mut matcher)).collect();
...
impl Score for Item {
        ...
        Self: Sized,
    {
        ...
        let scores =
            keywords.into_iter().map(|needle| pattern.score(Utf32Str::new(&needle, buf), matcher));
