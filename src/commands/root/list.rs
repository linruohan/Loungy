...
impl StateViewBuilder for RootListBuilder {
    fn build(&self, context: &mut StateViewContext, window: &mut Window, cx: &mut App) -> AnyView {
        context.query.set_placeholder("Search for apps and commands...", cx);
            ...
            .filter(move |this, cx| {
                ...
                if items.is_empty() {
                    if let Some(result) = numbat.read(cx).result.clone() {
                        items.push(
                            ItemBuilder::new("Numbat", NumbatWrapper { inner: numbat.clone() })
                            ...
                            .actions(vec![LAction::new(
                                ...
                                None,
                                {
                                    ...
                                    #[cfg(target_os = "macos")]
                                    {
                                        ...
                                        move |_, cx| {
                                            LWindow::close(cx);
                                            let result = if is_system_setting {
        ...
        cx.spawn(async move |cx| {
            ...
            for dir in dirs {
                let _ = debouncer.watcher().watch(&dir, notify::RecursiveMode::NonRecursive);
            }
