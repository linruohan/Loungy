...
use gpui::{
    AnyElement, AnyEntity, App, AppContext, Context, Entity, EventEmitter, FontWeight,
    InteractiveElement, IntoElement, ListAlignment, ListOffset, ListScrollEvent, ListState,
    MouseButton, ParentElement, Pixels, Render, RenderOnce, SharedString, Styled, Window, div,
    list, relative,
};
    ...
    pub fn reset_selection(&mut self, cx: &mut Context<Self>) {
        self.items.update(cx, |items, cx| {
            ...
            self.state.scroll_to(ListOffset {
                item_ix: s,
                offset_in_item: Pixels::from(0.0),
            })
        });
    }
